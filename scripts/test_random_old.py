...
def modo_random(serial):
    ...
    operaciones = c3_generar_operaciones( cantidad_comandos , C3_OPERATIONS )
...
def modo_unitario(serial):
    ...
    while( opt != "q" ):
        ...
        if( opt=="1"):
            ...
            enviar.append(cmd)
            #respuestas , tiempos = send_frames_3( serial , enviar , T_INTER_FRAME_LARGUISIMO )
            print('Enviar:')
            print(enviar)
        ...
        if( opt=="2"):
            ...
            print(cmd)
            enviar.append(cmd)
            #respuestas , tiempos = send_frames_3( serial , enviar , T_INTER_FRAME )
            #print(respuestas)
            if(len(recibido)==1 and recibido[0]==expected):
                print("PASS")
            else:
                print("FAIL" + " received:" + recibido[0] + " expected: "+  expected)
...
def uart_main():
    ...
    if ser.isOpen():
        ...
        try:
            ...
            th_tx = threading.Thread(target=TransmitThread(ser), args=(info,))
            th_rx = threading.Thread(target=ReceiveThread(ser), args=(info,))
